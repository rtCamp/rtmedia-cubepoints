...
/**
 * Description of RTMPluginInstaller
 *
 * @author sanket
 */
...
if( ! class_exists( 'RTMPluginInstaller' ) ){
	class RTMPluginInstaller{
		...
		function install_plugin( $plugin_slug ) {
			...
			$upgrader = new Plugin_Upgrader( new RTMedia_Plugin_Upgrader_Skin( array( 'nonce' => 'install-plugin_' . $plugin_slug, 'plugin' => $plugin_slug, 'api' => $api, ) ) );
		...
		function get_plugin_path( $plugin_slug ) {
			$filename = ( !empty( $this->plugins[ $plugin_slug ][ 'filename' ] ) ) ? $this->plugins[ $plugin_slug ][ 'filename' ] : $plugin_slug . '.php';
