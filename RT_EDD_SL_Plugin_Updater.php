...
if ( !class_exists( 'RT_EDD_SL_Plugin_Updater' ) ) {
	...
	class RT_EDD_SL_Plugin_Updater {
		...
		/**
		 * show update nofication row -- needed for multisite subsites, because WP won't tell you
		 * otherwise!
		 *
		 * @param string $file
		 * @param array  $plugin
		...
		function plugins_api_filter( $_data, $_action = '', $_args = null ) {
			...
			$to_send      = array( 'slug' => $this->slug, 'is_ssl' => is_ssl(), 'fields' => array( 'banners' => false /* These will be supported soon hopefully */, 'reviews' => false, ) );
		...
		private function api_request( $_action, $_data ) {
			...
			$api_params = array( 'edd_action' => 'get_version', 'license' => $data[ 'license' ], 'name' => isset( $data[ 'item_name' ] ) ? $data[ 'item_name' ] : false, 'item_id' => isset( $data[ 'item_id' ] ) ? $data[ 'item_id' ] : false, 'slug' => $data[ 'slug' ], 'author' => $data[ 'author' ], 'url' => home_url() );
